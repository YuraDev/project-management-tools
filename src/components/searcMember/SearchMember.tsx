    ...
    return (
        <>
          <input
            type="text"
            value={searchTerm}
            className="px-[12px] py-[8px] border border-[#ccc] rounded-[8px] mb-[16px] w-full text-[16px]"
            placeholder="Add user by ID..."
            onChange={handleSearchInput}
            onKeyDown={handleKeyDown}
          />
          {isLoading && <h1 className="px-[4px] py-[8px] flex justify-between items-center border-b mb-[45px]">Loading...</h1>}
          {isError && <h1 className="px-[4px] py-[8px] flex justify-between items-center border-b mb-[45px]">No search results</h1>}
          {!isLoading && !isError && searchedUser && (
            <div
              className="px-[4px] py-[8px] flex justify-between items-center border-b mb-[45px]"
